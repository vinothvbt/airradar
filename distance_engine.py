...
class DistanceCalculationEngine:
    ...
    def _calculate_log_normal_distance(
            self,
            signal_dbm: float,
            frequency_mhz: int,
            tx_power_dbm: float) -> float:
        ...
        try:
            ...
            env_params = self.params.get(
                "environmental_factors", {}).get(
                "indoor", {})
            n = env_params.get("path_loss_exponent", 2.0)  # Path loss exponent
            d0 = env_params.get(
                "reference_distance",
                1.0)   # Reference distance (m)
            pl_d0 = env_params.get(
                "reference_loss",
                40.0)   # Path loss at d0 (dB)
    ...
    def _calculate_itu_indoor_distance(
            self,
            signal_dbm: float,
            frequency_mhz: int,
            tx_power_dbm: float) -> float:
    ...
    def estimate_max_range(
        ...
        max_distance = self.calculate_distance(
            min_signal, frequency_mhz, tx_power_dbm)

        return max_distance

    def get_calculation_info(self, signal_dbm: float,
                             frequency_mhz: int) -> Dict[str, str]:
