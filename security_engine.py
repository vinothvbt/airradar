...
class SecurityAnalysisEngine:
    ...
    def analyze_security(self, ssid: str, security: str) -> Dict[str, str]:
        ...
        result = self.analyze_access_point(
            ssid, "00:00:00:00:00:00", security, -50.0, 2400)
        return {
            "threat_level": result.threat_level,
            "vulnerability_score": result.vulnerability_score
        }

    def analyze_access_point_dict(
            self, ap_dict: Dict[str, any]) -> Dict[str, any]:
        ...
        result = self.analyze_access_point(
            ssid, bssid, encryption, signal, frequency)
        return {
            "threat_level": result.threat_level,
            "vulnerability_score": result.vulnerability_score,
    ...
    def _analyze_access_point_full(
            self,
            ssid: str,
            bssid: str,
            security: str,
            signal_dbm: float,
            frequency: int) -> SecurityAnalysisResult:
        ...
        signal_bonus, signal_factors = self._analyze_signal_strength(
            signal_dbm)
        base_score += signal_bonus
        risk_factors.extend(signal_factors)

        # SSID analysis
        ssid_bonus, ssid_factors, ssid_recommendations = self._analyze_ssid(
            ssid)
        ...
        threat_level, threat_color = self.config.get_threat_level_for_score(
            final_score)
    ...
    def _analyze_signal_strength(
            self, signal_dbm: float) -> Tuple[int, List[str]]:
        ...
        if signal_dbm > -30:
            ...
            factors.append(
                "Fair signal strength - accessible with positioning")
        elif signal_dbm > -80:
    ...
    def _analyze_frequency(self, frequency: int) -> List[str]:
        ...
        if 2400 <= frequency <= 2500:
            ...
            factors.append(
                "6 GHz band - latest standard, limited device support")
        else:
    ...
    def _get_security_recommendations(self, security: str, signal_dbm: float,
        ...
        if signal_dbm > -30:
            recommendations.append(
                "Consider reducing transmit power to limit range")
    ...
    def get_analysis_summary(
            self, results: List[SecurityAnalysisResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "high_risk_percentage": round(
                (threat_counts["CRITICAL"] + threat_counts["HIGH"]) / total_aps * 100,
                1) if total_aps > 0 else 0}
